...
count_reps(dataset=bench_set, cutoff=0.4)
count_reps(dataset=squat_set, cutoff=0.35)
count_reps(dataset=row_set, cutoff=0.6, column="gyr_x")
count_reps(dataset=ohp_set, cutoff=0.35)
count_reps(dataset=dead_set, cutoff=0.4)
...
for s in df["set"].unique():
    ...
    reps = count_reps(dataset=subset, cutoff=cutoff, column=column)
    rep_df.loc[rep_df["set"] == s, "reps_pred"] = reps

# Evaluate the results

error = float(mean_absolute_error(rep_df["reps"], rep_df["reps_pred"]).round(2))
